...
(function(){
       ...
       // 新しい問題
	    ...
	    for (var i = 0; i < 5; i++){
	        ...
	        if (syouhin[r][3] == 'ok'){
	            ...
	            countNG++;
	            if (countNG > 2){
	                while (syouhin[r][3] != 'ok') {
	                    r = Math.floor( Math.random() * data.length);
	                }
	                countOK++;
	            }
	        }
	        fl_name[i] = syouhin[r][0];
	        s_name[i] = syouhin[r][1];
	        kingaku[i] = syouhin[r][2];
	    }
    ...
    function chk_card() {
        ...
        if(Math.abs(event.clientX - b1_x) < event.target.offsetWidth/2 + b1_w/2 //横の判定
        && Math.abs(event.clientY - b1_y) < event.target.offsetHeight/2 + b1_h/2 //縦の判定
        ){
            //if (flg_card < 0){flg_card = event.target.id-1;}
            flg_card = event.target.id-1;
            seiretu();
        } else if (flg_card == event.target.id-1) {
            reset();
        } else {
            seiretu();
        }
        ...
        if (count < 10) {
            if (count < 9){
                ...
                if (Number(kingaku[flg_card]) <= 500) {
                    ...
                    if (seikai == 3){
                        for (var i = 0; i < 5; i++){
                            //if (zumi[i] == -1){zumi[i] = 1;}//３問正解の後ののこりの選択肢は○に変更
                            if (zumi2[i] == 2){zumi[i] = 2;}//２問×の あとの正解は×に変更
                        }
                        //////////////////////////////////////////////////////////////////
                        // データ保存
                        //////////////////////////////////////////////////////////////////
                        data_s = localStorage.data;
                        now = new Date();
                        YY1 = now.getFullYear();
                        MM1 = now.getMonth()+1;
                        DD1 = now.getDate();
                        ...
                        //document.getElementById('kaikei').value = "つぎのもんだい！";
                    }//次の問題へ！
                    ...
                    if (matigai==3){
                        for (var i = 0; i < 5; i++){
                            if (zumi[i] == 0){zumi2[i] = 2;}else{zumi2[i] = 0;}
                        }
                    }
                }
 ...
 */
    ...
    function mmove(e) {
        ...
        //document.getElementById("zzz").innerHTML = event.target.id;
        document.getElementById("www").innerHTML = event.clientX;
        document.getElementById("hhh").innerHTML = event.clientY;
...
*/
        ...
        if(b1_x < event.clientX & b1_x+b1_w > event.clientX & b1_y < event.clientY && b1_y+b1_h > event.clientY){
            document.getElementById("zzz").innerHTML = "OK";
        } else {
            document.getElementById("zzz").innerHTML = "NG";
        }

        //chk_card();// 指定範囲内にあるかの判定 
