...
function renderAnswer(wrapper) {
    ...
    if (mode === 'handwrite') {
        ...
        if (!isEditMode) {
            ...
            clearBtn.style.backgroundColor = '#f1f1f1';
            ...
            const startDraw = (e) => {
                ...
                ctx.lineWidth = parseInt(wrapper.dataset.thickness) || 4;
                ctx.lineCap = 'round';
                ctx.strokeStyle = '#333';
                const pos = getPos(e);
                currentStroke = [pos];
            ...
            const stopDraw = (e) => {
                ...
                if (currentStroke.length > 1) {
                    canvas.drawnStrokes.push(currentStroke);
                }
            };
        ...
        if (isEditMode) {
            const label = document.createElement('div');
            label.className = 'id-label';
            label.textContent = answerId + ' (手書き)';
            el.appendChild(label);
        }
        return;
    }
    ...
    if (type === 'line') {
        ...
        if (type === 'box') {
            ...
            el.addEventListener('dblclick', (e) => {
                ...
                document.getElementById('box-prop-fontsize').value = wrapper.dataset.fontSize || "1.0"; 
            ...
            wrapper.dataset.thickness = itemData ? (itemData.thickness || 4) : 4; 
            ...
            const ansHandler = (e) => {
                ...
                document.getElementById('ans-prop-thickness').value = wrapper.dataset.thickness || 4; 
