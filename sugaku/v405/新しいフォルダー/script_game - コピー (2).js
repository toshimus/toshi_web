...
function runValidation(isAfterCorrection = false) {
    ...
    answers.forEach(wAns => {
        ...
        if (wAns.dataset.calcMode === 'handwrite') {
            const canvas = wAns.querySelector('canvas');
            if (!canvas || !canvas.drawnStrokes || canvas.drawnStrokes.length === 0) {
                hasEmpty = true;
            } else {
                if (typeof window.Tegaki !== 'undefined') {
                    const blocks = window.Tegaki.processStrokes(canvas.drawnStrokes);
                    if (!blocks || blocks.length === 0) {
                        if (canvas.drawnStrokes && canvas.drawnStrokes.length > 0) {
                            ansValues[normalizedId] = NaN; 
                            let rawX = Array(64).fill(0);
                            if (window.Tegaki.extractFeaturesFromStrokes) {
                                rawX = window.Tegaki.extractFeaturesFromStrokes(canvas.drawnStrokes, canvas);
                            }
                            tegakiContexts.push({ 
                                wrapper: wAns, 
                                blocks: [{
                                    role: 'number',
                                    strokes: canvas.drawnStrokes.flat(),
                                    X: rawX,
                                    candidates: [{digit: "?", distance: 0}], 
                        ...
                        const valStr = blocks.map(b => b.currentSelected !== null ? b.currentSelected : '').join('');
                        ansValues[normalizedId] = isNaN(parseFloat(valStr)) ? NaN : parseFloat(valStr);
                        tegakiContexts.push({ wrapper: wAns, blocks: blocks });
                    }
                }
            }
            return; 
    ...
    if (window.enableEmptyCheck === true) {
        let groupStatus = {}; 
        answers.forEach(w => {
            if (w.dataset.calcMode === 'handwrite') return; 
    ...
    if (hasEmpty) {
        ...
        if (typeof AppLogger !== 'undefined') {
            AppLogger.save({
                storageKey: "grid_custom_history",
                problemType: window.quizTitle || "自作グリッド問題",
                problemContent: problemContentText,
        ...
        if (!allCorrect) {
            if (!isAfterCorrection && tegakiContexts.length > 0 && typeof window.Tegaki !== 'undefined') {
                window.Tegaki.showCorrectionUI(tegakiContexts, () => {
        ...
        const chkSpeech = document.getElementById('speech-checkbox');
        const speechEnabled = (!chkSpeech || chkSpeech.checked) && typeof AppAudio !== 'undefined';

        if (allCorrect) {
            window.showToast(window.judgeSettings.correct.text, "correct");
            
            if (soundEnabled) {
                if (window.judgeSettings.correct.soundData) {
                    window.playSound(window.judgeSettings.correct.soundData);
                } else if (typeof AppAudio !== 'undefined') {
                    AppAudio.playOk();
                }
            }
            if (speechEnabled) {
                AppAudio.speak(window.judgeSettings.correct.text);
            }
            ...
            if (soundEnabled) {
                if (window.judgeSettings.incorrect.soundData) {
                    window.playSound(window.judgeSettings.incorrect.soundData);
                } else if (typeof AppAudio !== 'undefined') {
                    AppAudio.playNg();
                }
            }
            if (speechEnabled) {
                AppAudio.speak(window.judgeSettings.incorrect.text);
            }
...
window.showResultScreen = function() {
    ...
    if (!resultOverlay) {
        ...
        const appendTarget = document.querySelector('.app-body') || document.body;
        appendTarget.appendChild(resultOverlay);
    }
