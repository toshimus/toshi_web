...
addClick('export-html-btn', async () => {
    // 1. まず確実に現在のページの最新状態をproblemSetに保存する
    if (typeof window.saveCurrentPage === 'function') {
        window.saveCurrentPage();
    }

    // 2. その直後の最新データを含んだレイアウトデータを生成する
    const data = generateLayoutData();
    ...
    try {
        ...
        htmlClone.querySelector('head').appendChild(cssLink1);

        const cssLink2 = document.createElement('link');
        cssLink2.rel = 'stylesheet';
        cssLink2.href = 'app-common.css';
        htmlClone.querySelector('head').appendChild(cssLink2);

        const styleTag = document.createElement('style');
        styleTag.textContent = `
        body.run-mode { background-color: #222 !important; display: block !important; margin: 0; }
        #canvas-wrapper { width: 100vw; height: 100vh; display: flex; justify-content: center; align-items: center; overflow: hidden; }
        .app-body { width: 1024px; height: 768px; background-color: ${window.bgColor || '#ffffff'}; position: relative; transform-origin: center center; overflow: hidden; box-shadow: 0 0 20px rgba(0,0,0,0.8); border-radius: 10px; --grid-cell-w: calc(1024px / 32) !important; --grid-cell-h: calc(768px / 24) !important; }
        .app-body .box-container { width: 1024px !important; height: 768px !important; }
        .app-body .main-container { width: 100%; height: 100%; justify-content: center; align-items: center; }
        `;
        ...
        if (mainContainer) {
            ...
            const topNavHtml = `
                <div class="top-nav" style="position:absolute; top:0; left:0; width:100%; display:flex; justify-content:space-between; align-items:center; padding:10px 20px; box-sizing:border-box; background:rgba(255,255,255,0.9); z-index:9999; border-bottom:1px solid #ccc;">
                    <div class="toggle-group" style="display:flex; gap:15px; font-weight:bold; font-size:1.1rem; color:#333;">
                    ...
                    <a href="index.html" class="back-btn" style="text-decoration:none; padding:8px 20px; background-color:#e74c3c; color:white; border-radius:8px; font-weight:bold; font-size:1.1rem;">メニューへ戻る</a>
                </div>
            `;
            appBody.innerHTML = topNavHtml;
            
            mainContainer.parentNode.insertBefore(canvasWrapper, mainContainer);
            appBody.appendChild(mainContainer);
            canvasWrapper.appendChild(appBody);

            const startScreenHtml = `
            <div id="start-screen" class="start-screen" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-color: #f0f8ff; display: flex; flex-direction: column; justify-content: center; align-items: center; z-index: 10000;">
            ...
            `;
            appBody.insertAdjacentHTML('beforeend', startScreenHtml);
        }

        const extJsFiles = ['app-common.js', 'script_core.js', 'script_element.js', 'script_game.js', 'script_drag.js', 'script_tools.js', 'script_tegaki.js', 'script_main.js'];
        for (const file of extJsFiles) {
            const scriptEl = document.createElement('script');
            scriptEl.src = file;
            htmlClone.querySelector('body').appendChild(scriptEl);
        }
        ...
        function resizeCanvas() {
            ...
            const scaleX = wrapper.clientWidth / 1024;
            const scaleY = wrapper.clientHeight / 768;
            currentScale = Math.min(scaleX, scaleY);
            appBody.style.transform = 'scale(' + currentScale + ')';
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if(startBtn) {
                startBtn.addEventListener('click', () => {
                    ...
                    const soundEnabled = !chk || chk.checked;
                    if (soundEnabled) {
                        if (window.actionSoundData && typeof window.playSound === 'function') {
                            window.playSound(window.actionSoundData);
                        } else if (typeof AppAudio !== 'undefined') {
                ...
                startBtn.addEventListener('mousedown', function() { this.style.transform = 'translateY(8px)'; this.style.boxShadow = 'none'; });
                startBtn.addEventListener('mouseup', function() { this.style.transform = 'none'; this.style.boxShadow = '0 8px 0 #2980b9'; });
                startBtn.addEventListener('touchstart', function() { this.style.transform = 'translateY(8px)'; this.style.boxShadow = 'none'; });
                startBtn.addEventListener('touchend', function() { this.style.transform = 'none'; this.style.boxShadow = '0 8px 0 #2980b9'; });
            }
        });
        `;
