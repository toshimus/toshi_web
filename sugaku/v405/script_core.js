...
window.judgeSettings = {
    correct: { text: "せいかい！", color: "#ff3333", stroke: "#000000", strokeWidth: 6, bg: "transparent", soundData: null },
    incorrect: { text: "おしい！", color: "#3366ff", stroke: "#000000", strokeWidth: 6, bg: "transparent", soundData: null }
};
window.actionSoundData = null;

window.playSound = function(base64Data) {
    if (base64Data) {
...
overlay.addEventListener('click', () => {
    ...
    const actionSet = document.getElementById('action-settings-container');
    if (actionSet) actionSet.style.display = 'none';
...
const modals = [
    calcContainer, 
    document.getElementById('action-settings-container'), 
...
function showToast(message, type = 'system') {
    ...
    if (type === 'correct' || type === 'incorrect') {
        ...
        // 入力されたふちの太さ(strokeWidth)を反映させる
        const width = settings.strokeWidth !== undefined ? settings.strokeWidth : 6;
        toast.style.webkitTextStroke = `${width}px ${settings.stroke}`;
